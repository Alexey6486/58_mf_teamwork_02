...
export const getError = (meta: FieldMetaProps<unknown>, helperText?: string | ReactNode): IError => {
  ...
  const errorText = isArray(text)
    ...
    : isObject(text)
      ? Object.values(text).filter(Boolean).map((item, index) => (<span key={index + item}>{item}</span>))
      : text;
